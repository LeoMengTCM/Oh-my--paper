...
**但光加参数还不够——这个手势只有约 5 秒。** 实测在同一页面、同一条链接上，
先在求值里 `await` 一段时间再点：
...
所以「先 `await waitUntil(...)` 等详情页就绪、再 `click()`」这种写法是不可靠的：
`download --url` 刚 `navigate` 完要整页加载，等页面就绪 + `guardCaptcha` 的往返很容易
超过 5 秒，点击就会被静默丢掉，而命令仍然返回 `status: downloading`。
**实测就是这么翻车的**：同一篇论文，页面已缓存时能下、需要真加载时下不了。

`cmdDownload` 现在拆成两次求值：第一次只等待与判定（返回该点哪个链接），
第二次**只点击、前面不许有任何 await**。别再把它们合回一个求值里。
...
### 17. 翻页也会整页跳转，不要在同一个求值里等

和 `journal` 点检索按钮是同一个坑：点翻页链接会触发整页跳转，执行上下文随之销毁，
在同一个求值里 `await waitUntil(页码变化)` 永远不 resolve，表现为
`CDP 命令超时: Runtime.evaluate`。它是**偶发**的，所以很容易被当成网络抖动放过去。

还有个必然翻车的分支：**点当前页是空操作**，页码不会变，等下去必然落空
（和「点已激活的排序项」同类）。`pages --action page:1` 停在第一页时就会踩到。

`cmdPages` 现在的写法：第一次求值只判断（已在目标页就直接返回 `alreadyThere`，
找不到页码就报错并列出可点的页），点击后立刻返回；页码变化由 Node 侧轮询，
每次都是新的求值；最后再解析结果。
