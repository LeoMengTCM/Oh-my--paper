...
- If a target venue is named but no local venue guide exists, use the NeurIPS LaTeX fallback and state that the final format must be checked by `submission-checker`.
...
Use with `score-lifting-loop.md` and, only when explicitly requested or allowed by the CCFA handoff mode, `paper-reviewer` for scientific or writing review. Use `inno-deep-research`, `inno-experiment-dev`, or `paper-writing` compression mode through the same handoff rule when the blocker is missing prior art, evidence design, or page-limit pressure.
