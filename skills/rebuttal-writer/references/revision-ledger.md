...
Use this reference when rebuttal, response-letter, resubmission, or camera-ready work needs a traceable ledger. The ledger is now part of `rebuttal-writer`; it is not a separate runtime skill.
...
| `owner_skill` | Usually `rebuttal-writer`, `paper-writing`, `inno-experiment-dev`, or `submission-checker`. |
