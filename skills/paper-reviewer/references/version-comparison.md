...
The project retains one canonical ledger. The reviewer returns ledger-ready rows with stable IDs; `rebuttal-writer` updates that ledger in place. Each issue needs these fields:
...
Write the comparison into the canonical review report. Return ledger-ready issue rows and hand them to `rebuttal-writer`, which owns updates to the canonical revision ledger. Do not create phase reports, response-reconciliation sidecars, or per-round copies unless the user explicitly asks for them.
