...
- `paper-reviewer` owns simulated reviewers, AC/meta-review, scientific scoring, version comparison, and both cross-version scorecards.
- `paper-writing` owns manuscript edits, section restructuring, compression, and prose polish.
- `inno-experiment-dev`, `inno-deep-research`, `inno-figure-gen`, and `integrity-auditor` own evidence, literature, figures, and integrity checks respectively.

If the user asks for reviewer-style critique or a score, hand the manuscript to `paper-reviewer`. Do not simulate a parallel reviewer panel inside the writing task.
...
5. Apply `references/prose-quality-guardrails.md` and the `paper-humanization` policy.
...
Formal issue provenance, historical/current score deltas, and the canonical revision ledger remain with `paper-reviewer` and `rebuttal-writer`. Do not create writer-owned review scores or per-pass critique files.
