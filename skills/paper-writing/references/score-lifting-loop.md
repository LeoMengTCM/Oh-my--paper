...
`paper-reviewer` owns scientific scoring, reviewer simulation, score calibration, and cross-version comparison. For a revision comparison it keeps two separate systems:
...
8. Return unresolved findings to `paper-reviewer` for re-evaluation when the user requests a new score.
