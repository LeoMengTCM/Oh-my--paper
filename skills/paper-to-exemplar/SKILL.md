...
that the `paper-writing` skill can use as style references. This skill does not
...
5. Load `../paper-writing/references/prose-quality-guardrails.md` and filter the extracted pattern: preserve transferable logic, but do not turn defensive framing, `Q1`/`C1` labels, formula dumping, number-only abstracts, third-person manuscript narration, punctuation-heavy structure, or unsupported hype into reusable advice.
6. Fill in the `[ANALYZE]` placeholders with concrete, actionable writing-pattern descriptions.
7. Save the completed card to `paper-writing/references/exemplars/cards/`.
8. Register the card in `paper-writing/references/exemplars/index.md` under the correct venue section.
9. If the user wants this as their default writing template, update `paper-writing/references/custom-format/default-user-format.md`.
...
1. Classify the target venue into its venue family using `../paper-writing/references/venue-guides/index.md`.
...
3. If the user does NOT provide a PDF but names a venue, check whether exemplar cards already exist for that venue:
 - Look in `../paper-writing/references/exemplars/index.md` for venue-matched bundles.
...
paper-to-exemplar (distill PDFs -> create cards -> update index)
 -> paper-writing (load cards from index -> write with venue-matched style)
```
...
5. Provide next-action options: "Run paper-writing with these exemplars" or "Add more papers."
...
- `../paper-writing/references/exemplars/index.md`: exemplar registry (this skill writes to it).
- `../paper-writing/references/exemplars/cards/`: card storage directory.
- `../paper-writing/references/custom-format/default-user-format.md`: default exemplar configuration.
- `../paper-writing/references/venue-guides/index.md`: venue family classification.
- `../paper-writing/references/prose-quality-guardrails.md`: anti-pattern filter for distilled writing cards.
...
Venue/year: CVPR2025.
Source: distilled from PDF by paper-to-exemplar.
