...
使用评审 skill 对项目 LaTeX 论文进行同行评审——ml / bioinformatics track 用 skills 下的 `paper-reviewer/SKILL.md`（venue 导向的科学评审、评分、录用准备度、跨版本对比）；clinical / systematic-review track 用 `inno-paper-reviewer/SKILL.md`（CONSORT/STROBE/STARD/PRISMA 合规评审）。并按第零步确定的 track 报告规范清单逐条核对（缺项列入"必须修改"）。额外核对：①图表充分性——对照 `.pipeline/memory/figure_ledger.md` 和 `experiment_ledger.md`，每个主要结果是否有图、图是否多面板组织、caption 是否自含；②篇幅完整性——experiment_ledger 每条是否都在正文有对应小节，有没有被一句话带过的工作量。将报告追加写入 `.pipeline/memory/review_log.md`，格式：评分表格 + 必须修改列表 + 建议修改列表 + 推荐结论。完成后更新 `agent_handoff.md`。
