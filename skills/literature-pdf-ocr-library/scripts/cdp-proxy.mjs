#!/usr/bin/env node
// CDP Proxy — 通过 HTTP API 操控用户日常 Chrome，用于访问需要浏览器自动化的学术平台
// （Google Scholar 等）。纯 Node 内置依赖，无 npm 包。
...
//
// ⚠️ 这个手势**只管约 5 秒**，调用方必须自己保证"点击"紧跟在求值开始处。
// 实测同一页面、同一链接，在求值里先 await 若干毫秒再点：
//     延迟 2s → 下载成功    延迟 4s → 下载成功
//     延迟 6s → 点了没反应  延迟 8s → 点了没反应
// 所以「先 await 等页面就绪、再 click」这种写法是不可靠的：页面慢一点就超时，
// 表现为"返回已触发、实际没有下载"。正确做法是把等待与判定放进一次求值、
// 把 click 单独放进另一次求值（前面不许有任何 await）。
// cnki.mjs 的 cmdDownload 就是这么写的，别再合回去。
