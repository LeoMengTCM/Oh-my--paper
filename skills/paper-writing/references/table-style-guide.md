...
5. **Hyphenate long monospace words**: in narrow columns, long identifiers like `inno-experiment-dev` will overflow even with `\raggedright`. Abbreviate to a shorter form (e.g., `inno-experiment-dev`) and explain in the caption or a table note. Do not let the compiler produce overfull hboxes from unbreakable words.
