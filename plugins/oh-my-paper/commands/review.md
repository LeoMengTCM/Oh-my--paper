...
```
/codex:rescue --background 使用 .claude/skills/inno-paper-reviewer/SKILL.md 对项目 LaTeX 论文进行同行评审（[含用户额外要求]），并按第零步确定的 track 报告规范清单逐条核对（缺项列入"必须修改"）。将报告追加写入 .pipeline/memory/review_log.md，格式：评分表格 + 必须修改列表 + 建议修改列表 + 推荐结论。完成后更新 agent_handoff.md。
```
