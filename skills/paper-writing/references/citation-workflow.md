...
- **Topic gap:** Route to `inno-deep-research` for a targeted search. Specify: task area, year range, venue preference, and what the citation should support.
...
- **Before drafting:** If the paper's closest competitors, baselines, or related-work clusters are unknown, route to `inno-deep-research` first. Do not draft with guessed citations.
...
- **After drafting:** Route to `integrity-auditor` to verify that every citation exists and every BibTeX entry is complete.
- **Before submission:** Route to `submission-checker` to verify the `.bib` file compiles cleanly with the venue's LaTeX template.
