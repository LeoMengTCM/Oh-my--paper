...
- If closest work is unknown or fast-moving, use `inno-deep-research` through the CCFA handoff mode rather than inventing citations.
...
- If the user needs datasets, baselines, ablations, benchmark design, or fill-in result tables, use `inno-experiment-dev` through the CCFA handoff mode.
