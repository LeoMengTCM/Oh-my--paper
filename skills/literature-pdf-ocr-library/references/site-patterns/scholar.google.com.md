...
> 改编自 academic-search skill（MIT, Copyright (c) 2026 Chengmingyue）：https://github.com/ustc-ai4science/academic-search
>
> 本仓库把 CDP 能力收在 `skills/literature-pdf-ocr-library/scripts/`（`cdp-proxy.mjs` + `check-deps.sh`）。Google Scholar 在 survey 流程里是**可选增强**，仅用于补最全引用数 / 发现其他源未收录的论文；没有 Chrome 远程调试时 `check-deps.sh` 会优雅跳过，主检索（arXiv/S2/OpenAlex/Crossref/PwC）照常进行。
...
```bash
# 先确保 CDP Proxy 就绪（脚本会按需启动 cdp-proxy.mjs）
bash "$(dirname "$0")/check-deps.sh"   # 或: bash skills/literature-pdf-ocr-library/scripts/check-deps.sh
...
- 引用数 "Cited by N" 中的 N 与 Semantic Scholar 的数值通常不一致（Scholar 更高，包含更多灰色文献）；合并时 `citation_count` 优先取 Google Scholar
