...
Apply this skill before manuscript-facing work by `paper-writing` and before publication-facing experiment work by `inno-experiment-dev`. It is the highest-priority preflight as a sidecar quality layer, not a substitute for either owner. Respect the user's requested format and do not broaden the research scope.
